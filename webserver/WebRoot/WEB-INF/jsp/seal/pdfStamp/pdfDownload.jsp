					...
					<div class="x_panel">
						<div class="x_title" id="x_title">
							<div class="col-md-4 col-sm-4 col-xs-4">
								...
								<input type="button" value="下载"  onclick="downloadPdf()"  class="btn btn-primary" style=" float:left; margin-left:10px;" data-target=".bs-example-modal-sm12"/>
								<input type="button" value="删除"  onclick="delStampPdf()"  class="btn btn-primary" style=" float:left; margin-left:10px;" data-target=".bs-example-modal-sm12"/>
						...
						<div class="x_content">
							<div id="table-box" style="display: block;">
								<c:forEach items="${stamPdf }" var="stamPdf">
									<div style="float: left;margin: 8px;" align="center">
										<img id="timg" src="${ctx }/img/folder.jpg" width="80px" height="80px"/>
										<p>
											<input id="ipdf" name="checkboxt" class="input_checked" type="checkbox" value="${stamPdf }">
											${stamPdf }
										</p>
									</div>
								</c:forEach>
...
<script type="text/javascript">
	...
	function downloadPdf(){
		var ids = "";
		var index = 0;
		$("[id='ipdf']:checkbox").each(function() {
			if (this.checked) {
				if (index == 0) {
					ids = $(this).val();
				} else{
		...
		$.ajax({
			url :"${ctx }/sealManage/pdfStamp/toDownloadStampPdf.do",
			type : "post",
			data : "ids=" + ids,
			dataType : "json",
			success : function(data) {
				if(data.success){
					window.location.href="${ctx }/sealManage/pdfStamp/downloadPdf.do?zipPath=" + data.message;
				}else{
	...
	function delStampPdf(){
		...
		layer.confirm("确定要删除文件夹及文件?",{btn:["确定","取消"]},function(){	
			$.ajax({
				url :"${ctx }/sealManage/pdfStamp/delStampPdfFolder.do",
				type : "post",
				data : "ids=" + ids,
				dataType : "json",
				success : function(data) {
	...
	function viewFolder(){
		...
		loadTab("${ctx }/sealManage/pdfStamp/viewStampFolder.do?ids=" + ids, "tab3");
	}
