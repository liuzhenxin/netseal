					...
					<div class="x_panel">
						<div class="x_title" id="x_title">
							<div class="col-md-10 col-sm-10 col-xs-10">
								<input id="allS" type="button" value="全选" class="btn btn-primary" style=" float:left; margin-left:10px;" data-target=".bs-example-modal-sm12"/>
								...
								<input type="button" value="文件下载"  onclick="downloadPdf()"  class="btn btn-primary" style=" float:left; margin-left:10px;" data-target=".bs-example-modal-sm12"/>
							</div>
							...
							<div id="table-box" style="display: block;">
								<c:forEach items="${files }" var="files">
									<div style="float: left; margin: 10px;" align="center">
										...
										<p>
											<input id="idir" name="checkboxt" class="input_checked" type="checkbox" value="${files }">
											${files }
										</p>
									</div>
								</c:forEach>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myModalLabel">PDF批签</h4>
            </div>
							...
							<div class="col-md-12 col-sm-12 col-xs-12">
								<div class="x_panel">
									<form id="" action="" data-parsley-validate="" class="form-horizontal form-label-left" novalidate style="margin-top: 20px;">
										...
										<div class="form-group" id="pdf_Qfz">
											<label class="control-label col-xs-3">骑缝位置</label>
											<div class="col-xs-6" style="margin-top: 7px">
												<input type="radio" name="QFZ" id="QFZL" value="0"><label for="QFZL">左骑缝</label>
												<input type="radio" name="QFZ" id="QFZR" value="1" style="margin-left: 20px;" ><label for="QFZR">右骑缝</label>
											</div>
										</div>
										<br>
										<div class="form-group">
											<button id="stampPdf" onclick="batchStamp()" type="button" class="btn btn-primary col-md-offset-4">批签</button>
...
<script type="text/javascript">
	...
	$(function(){
		...
		document.getElementById("pdf_Qfz").style.display = "none";//隐藏
	});
	...
	$("#types").change(function() {
		...
		if (optValue == 1) { //关键字盖章
			...
			document.getElementById("pdf_Qfz").style.display = "";
			document.getElementById("keyW").style.display = "none";
	...
	function batchStamp() {
		...
		var ids = "";
		var index = 0;
		$("[id='idir']:checkbox").each(function() {
			if (this.checked) {
				if (index == 0) {
					ids = $(this).val();
				} else {
		...
		$.ajax({
			url : "${ctx }/sealManage/pdfStamp/pdfBatchStamp.do",
			type : "post",
			data : {
				"ids" : ids,
				"name" : name,
				...
				"X" : $("#X").val(),
				"Y" : $("#Y").val(),
				"QFZ" : $('input[name="QFZ"]:checked').val()
			},
			dataType : "json",
			success : function(data) {
				if (data.success) {
					...
					loadTab("${ctx }/sealManage/pdfStamp/pdfBatchStampManage.do", "tab3");
				}
			},
			error : function() {
				$(".modal-backdrop").remove();
