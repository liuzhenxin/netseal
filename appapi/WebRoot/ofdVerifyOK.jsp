...
<%
	...
	if (!path.endsWith(Constants.OFD_SUFFIX))
		throw new Exception("file suffix must " + Constants.OFD_SUFFIX);

	byte[] data = FileUtil.getFile(path);
	String base64 = StringUtil.base64Encode(data);
	
	String checkSealDate = request.getParameter("OFD_CHECK_SEAL_DATE");
	String checkCertDate = request.getParameter("OFD_CHECK_CERT_DATE");
	
	Properties pro = new Properties();
	pro.setProperty(Constants.OFD_DATA, base64);
	pro.setProperty(Constants.OFD_CHECK_SEAL_DATE, checkSealDate);
	pro.setProperty(Constants.OFD_CHECK_CERT_DATE, checkCertDate);

	boolean result = false;
	try {
		result = NetSealClient.verifyOfdStamp(pro);
	} catch (Exception e) {
		throw e;
	}
%>
