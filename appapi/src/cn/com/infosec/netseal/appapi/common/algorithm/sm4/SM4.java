package cn.com.infosec.netseal.appapi.common.algorithm.sm4;
...
public class SM4 {
	...
	public final static int sm4_cbc_encrypt(byte[] in, int in_len_bytes, byte[] out, final byte[] key, final int key_len_bytes, final byte ivec[]) {
	...
	public final static int sm4_cbc_decrypt(final byte[] in, final int in_len_bytes, byte[] out, int out_len_bytes, final byte[] key, final int key_len_bytes, final byte ivec[]) {
		int i, j, numBlocks;
		byte tr[] = new byte[16], iv[] = new byte[16];
		int rk[] = new int[32];

		numBlocks = in_len_bytes >>> 4;
		if (key_len_bytes != 16)
			return SM4_BAD_KEY;
		sm4_key_exp(key, rk);
	...
	public final static int sm4_ecb_encrypt(final byte[] in, final int in_len_bytes, byte[] out, final byte[] key, final int key_len_bytes) {
		int i, j, numBlocks, pc;
		byte[] tr = new byte[16];
		int rk[] = new int[32];
	...
	public final static int sm4_ecb_decrypt(final byte[] in, final int in_len_bytes, byte[] out, int out_len_bytes, final byte[] key, final int key_len_bytes) {
		int i, j, numBlocks;
		byte[] tr = new byte[16];
		int[] rk = new int[32];

		numBlocks = in_len_bytes / 16;
		if (key_len_bytes != 16)
			return SM4_BAD_KEY;
		sm4_key_exp(key, rk);
	...
	private final static void validation() {
		byte[] key = { (byte) 0x01, (byte) 0x23, (byte) 0x45, (byte) 0x67, (byte) 0x89, (byte) 0xab, (byte) 0xcd, (byte) 0xef, (byte) 0xfe, (byte) 0xdc, (byte) 0xba, (byte) 0x98, (byte) 0x76,
		...
		sm4_key_exp(key, rk);
		// for (int i = 0; i < 32; i++)
		// System.out.println("rk[" + i + "]=" + String.format("%08x", rk[i]));
		// long start, end;
		for (int i = 0; i < 16; i++)
			pt[i] = key[i];
		...
		System.out.println("\nplaintext:");
		for (int i = 0; i < 16; i++)
			System.out.print(String.format("%02x ", pt[i]));
	...
	public static void main(String[] args) {
		...
		byte[] in = "5000000000000000013| 6/+0ixUB7xbfK3M/Q907YSvugSwLp40gBTBZdCTZdXI=@Wu4VH3a1imHbBjR4HUkLoQ==| IlNUkX0PMe86jHF1Fmh5nmGukXAruD6jHWs0f5tiOhM=".getBytes();
		// byte[] in = "12345678901234567".getBytes();

		byte[] out = new byte[in.length + (in.length % 16 == 0 ? 16 : 16 - (in.length % 16))];

		SM4.sm4_ecb_encrypt(in, in.length, out, key, key.length);
		System.out.println(HexUtil.byte2Hex(out));

		byte[] out_bs = new byte[1024];
		SM4.sm4_ecb_decrypt(out, out.length, out_bs, out_bs.length, key, key.length);

		// 去掉多余的0x00
		int len = out_bs.length;
		while (len > 0 && out_bs[len - 1] == 0)
			len--;
		byte[] plain = new byte[len];
		System.arraycopy(out_bs, 0, plain, 0, len);

		System.out.println(new String(plain));

		// byte[] key = { (byte) 0xe5, (byte) 0xb8, (byte) 0x7b, (byte) 0x4d,
		// (byte) 0xf5, (byte) 0xe8, (byte) 0x48, (byte) 0x20,
		// (byte) 0x54, (byte) 0x14, (byte) 0x48, (byte) 0xb1,
		// (byte) 0x1f, (byte) 0x5a, (byte) 0xa9, (byte) 0x3f };
		// // new byte[16];
		// byte IV[] = { (byte) 0x12, (byte) 0xa4, (byte) 0xb7, (byte) 0x39,
		// (byte) 0xee, (byte) 0x9a, (byte) 0xc2, (byte) 0x43,
		// (byte) 0x97, (byte) 0x24, (byte) 0xd8, (byte) 0xc0,
		// (byte) 0xf1, (byte) 0xa7, (byte) 0x39, (byte) 0x71 };
		//
		...
		// for (int i = 0; i < max; i++)
		// SM4.sm4_cbc_encrypt(p1k, 1024, c1k, key, SM4_BLOCK_SIZE, IV);
		// end = System.currentTimeMillis();
		// System.out.println("1k bytes input, Throughput ="
		// + String.format("%8.2f", (max * 1024 * 1.0 / 1024 / 1024
		// * 1000 / (end - start))) + " MBps");
		// start = System.currentTimeMillis();
		// for (int i = 0; i < max; i++)
		// SM4.sm4_cbc_encrypt(p8k, 8192, c8k, key, SM4_BLOCK_SIZE, IV);
		// end = System.currentTimeMillis();
		// System.out.println("8k bytes input, Throughput ="
		// + String.format("%8.2f", (max * 1024 * 8.0 / 1024 / 1024
		// * 1000 / (end - start))) + " MBps");
		// validation();
		// } catch (Exception e) {
