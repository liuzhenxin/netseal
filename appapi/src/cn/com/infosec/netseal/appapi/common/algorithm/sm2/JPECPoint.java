...
public abstract class JPECPoint {
	...
	public JPECPoint getInfinity() {
		return new JPECPoint.Fp(SM2.gmec256.getInfinity());
	}
	...
	ECPoint toECPoint() {
		...
		BigInteger zi = z.toBigInteger().modInverse(SM2.gmp);// zi=1/z
		BigInteger zi2 = zi.multiply(zi);
		ax = x.toBigInteger().multiply(zi2).mod(SM2.gmp);// x=x/z^2
		ay = y.toBigInteger().multiply(zi).multiply(zi2).mod(SM2.gmp); // y=y/z^3
